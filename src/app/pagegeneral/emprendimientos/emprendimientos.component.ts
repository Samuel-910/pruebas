  ...
  <!-- Sección de Asociaciones -->
             <!-- Sección de Asociaciones -->
<section class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 transition-colors duration-300">
  <div class="container mx-auto px-4 py-10">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
      ...
      <button
        ...
        class="flex flex-col items-center justify-center p-5 rounded-xl transition-all duration-300 border-2 border-transparent hover:border-orange-300 dark:hover:border-amber-500 focus:outline-none focus:ring-2 focus:ring-orange-400 dark:focus:ring-amber-400"
        title="Mostrar todos los emprendimientos"
      >
        ...
        <span class="text-xs mt-1 opacity-75">({{ emprendimientosOriginales().length }})</span>
      </button>

      <!-- Botón Independientes -->
      <button
        (click)="seleccionarIndependientes()"
        [class]="estadoFiltroActual() === 'independientes' ?
          'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg transform scale-105' :
          'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
        class="flex flex-col items-center justify-center p-5 rounded-xl transition-all duration-300 border-2 border-transparent hover:border-purple-300 dark:hover:border-pink-500 focus:outline-none focus:ring-2 focus:ring-purple-400 dark:focus:ring-pink-400"
        title="Mostrar solo emprendimientos independientes"
      >
        <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
        </svg>
        <span class="font-semibold">Independientes</span>
        <span class="text-xs mt-1 opacity-75">({{ emprendimientosIndependientes().length }})</span>
      </button>

      <!-- Botones dinámicos de asociaciones -->
      <ng-container *ngFor="let estadistica of estadisticasAsociaciones()">
        <button
          ...
          [class]="asociacionSeleccionada()?.id === estadistica.id && estadoFiltroActual() === 'asociacion' ?
            ...
            'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
          class="flex flex-col items-center justify-center p-5 rounded-xl transition-all duration-300 border-2 border-transparent hover:border-blue-300 dark:hover:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-indigo-400"
          [title]="'Asociación: ' + estadistica.nombre"
        >
          <img
            *ngIf="estadistica.imagen_url; else defaultIcon"
            [src]="estadistica.imagen_url"
            [alt]="estadistica.nombre"
            class="w-8 h-8 mb-2 rounded-full object-cover ring-2 ring-white dark:ring-gray-800"
          ...
          <span class="font-semibold text-sm truncate w-full text-center">{{ estadistica.nombre }}</span>
          <span class="text-xs mt-1 opacity-75">({{ estadistica.cantidadEmprendimientos }})</span>
        </button>
      </ng-container>
    </div>

    <!-- Indicador de filtros activos -->
    <div *ngIf="tieneFiltrosActivos()" class="mb-6 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-5 shadow-md border-l-4 border-orange-500 dark:border-blue-500">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-center space-x-3">
          <svg class="w-6 h-6 text-orange-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div>
            <p class="text-sm font-bold text-gray-800 dark:text-gray-200">
              Filtro aplicado: {{ obtenerDescripcionFiltros() }}
            </p>
            <p class="text-xs text-gray-600 dark:text-gray-400">
              Mostrando {{ emprendimientosFiltrados().length }} de {{ emprendimientosOriginales().length }}
            </p>
          </div>
        </div>
        <button
          (click)="limpiarFiltros()"
          class="flex items-center space-x-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200"
        >
    ...
    <div *ngIf="asociacionSeleccionada() && estadoFiltroActual() === 'asociacion'" class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 mb-6 shadow-lg border border-blue-100 dark:border-gray-600">
      <div class="flex flex-col sm:flex-row gap-5">
        <img
          *ngIf="asociacionSeleccionada()!.imagen_url"
          [src]="asociacionSeleccionada()!.imagen_url"
          [alt]="asociacionSeleccionada()!.nombre"
          class="w-20 h-20 rounded-full object-cover ring-4 ring-blue-200 dark:ring-blue-700"
        />
        <div class="flex-grow">
          ...
          <div class="flex flex-wrap gap-3 mt-2">
            <span *ngIf="asociacionSeleccionada()!.telefono" class="bg-white dark:bg-gray-700 px-3 py-1 rounded-full text-sm text-gray-700 dark:text-gray-200 flex items-center">
              📞 {{ asociacionSeleccionada()!.telefono }}
            </span>
            <span *ngIf="asociacionSeleccionada()!.email" class="bg-white dark:bg-gray-700 px-3 py-1 rounded-full text-sm text-gray-700 dark:text-gray-200 flex items-center">
              📧 {{ asociacionSeleccionada()!.email }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div *ngIf="estadoFiltroActual() === 'independientes'" class="bg-gradient-to-r from-green-50 to-teal-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 shadow-lg border border-green-100 dark:border-gray-600">
      <div class="flex flex-col sm:flex-row gap-5">
        ...
        <div>
          ...
          <p class="text-gray-600 dark:text-gray-300 text-sm">
            Estos son emprendimientos que operan de forma autónoma sin pertenecer a ninguna asociación.
          </p>
        </div>
      </div>
    </div>
  ...
  <div *ngIf="emprendimientoModal()"
      class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-start justify-center p-4 z-50 pt-8"
      (click)="cerrarModal()">
    <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-5xl w-full max-h-[85vh] overflow-auto shadow-2xl modal-content mt-8"
